...
Leave `shell` off and the entrypoint gets your terminal instead, as long as it is a process service. An entrypoint that only groups other services (a `depends_on:` list with no `process:`) has nothing to hand the terminal to, so fed asks you to pass one of them.
